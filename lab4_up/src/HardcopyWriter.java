...
public class HardcopyWriter extends Writer {
     ...
     **/
    public HardcopyWriter(String jobname, int fontsize,
    ...
    public void write(char[] buffer, int index, int len) {
        ...
        for(int i = index; i < index + len; i++) {
            ...
            if(isdraw)
            {
                char[] b={'У','л','и','т','к','а',' ','П','а','с','к','а','л','я'};
                BufferedImage bf=new BufferedImage(300,300,BufferedImage.TYPE_INT_RGB);
                Graphics2D tg = bf.createGraphics();
                tg.setColor(Color.white);
                tg.fillRect(0, 0, 400, 400);
                tg.setColor(Color.BLACK);
                tg.setStroke(new MyStroke(1));
                tg.draw(new MyPascal(0.03,150,75,300,130));
                page.drawImage(bf, 50, width/2-200, null);              // Set the header font.
                page.drawChars(b, 0, 14, 125, width-175);
                isdraw=false;
            }
